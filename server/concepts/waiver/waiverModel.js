...
export async function createWaiver(db, waiver) {
  const { student_user_id, waiver_type, related_entity_id, related_entity_type, description, granted_by_user_id, expires_at } = waiver;
  const sql = `
    INSERT INTO waivers (student_user_id, waiver_type, related_entity_id, related_entity_type, description, granted_by_user_id, expires_at)
    VALUES ($1, $2, $3, $4, $5, $6, $7)
    RETURNING waiver_id, student_user_id, waiver_type, related_entity_id, related_entity_type, description, granted_by_user_id, granted_at, expires_at
  `;
  const { rows } = await db.query(sql, [student_user_id, waiver_type, related_entity_id, related_entity_type, description, granted_by_user_id, expires_at]);
  return rows[0];
}
